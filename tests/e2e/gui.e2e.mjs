...
async function main() {
  ...
  try {
    ...
    const urlBefore = await win.locator('[data-testid="sync-pick-count"]').innerText()
    await win.locator('[data-testid="sync-rows"] .ant-checkbox').first().click()
    await win.waitForTimeout(400)
    assert((await win.locator('[data-testid="sync-pick-count"]').innerText()) !== urlBefore, 'url list uncheck shrinks set')
    await win.locator('[data-testid="sync-rows"] .ant-checkbox').first().click()
    await win.waitForTimeout(400)
    assert((await win.locator('[data-testid="sync-pick-count"]').innerText()) === urlBefore, 'url list recheck restores set')
