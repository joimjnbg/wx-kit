...
| **M59** ✅ | v0.11.0（进行中） | R1 文库单篇复制保存路径：卡片右键菜单（四项，antd Dropdown）+ 列表常驻按钮；plan 期核实 `meta.dir` 本身绝对路径、`copyText` IPC 已存在，零核心层/IPC 改动（2026-09-11 完成，589 单测 + e2e 七条新断言全绿） | `plans/2026-09-11-m59-copy-article-path.md` |
| **M60** ✅ | v0.11.0（进行中） | R3 启动检测 mocli（settings 三键 + 设置页「墨问集成」区块 + `mowen:detect` IPC，fire-and-forget 隔离）+ mocli 封装层（runner/detect/metadata/url，契约真机钉死）+ CLI 五命令（detect/search-user/list-user/list-mine/search，白名单+前置检测+指引）。真机验收五命令全过；两个暗坑实录（which 非子命令、失败 JSON 走 stderr）见 devlog（2026-09-11 完成，605 单测 + e2e 全绿） | `plans/2026-09-11-m60-mowen-detect-and-discovery.md` |
...
- **v0.10.3 · 2026-09-08 · 三个小修:CLI 入口守卫、文库筛选名称、识别反馈** —— M57 三件体验可信度修复:CLI wrapper 拒绝指向开发/构建目录的临时产物(2026-09-04 悬空实录的根治,打包实机验证守卫生效);文库筛选下拉在任何入口下显示公众号名称而非裸 ID(options 升级为身份 value/名称 label,跳转补传显示名覆盖 0 篇空态);订阅页「识别」全程有反馈(loading/空输入/零候选不再「点了没反应」)。需求收集阶段完成阅读数 spike 证伪(微信读书通道无计数字段,勿再投入)与公式保真推迟的决定(devlog §53)。发布说明 `docs/releases/v0.10.3.md`,复盘 devlog §53。
...
- **文章阅读数等计数:已证伪,勿再投入** —— 2026-09-07 spike 终局(实证脚本 `scripts/spike-weread-readcount.mjs`):微信原生阅读数需手机微信登录态(`getappmsgext`),匿名请求为空;微信读书侧唯一活接口 cover 完整 payload 无任何计数字段,其余候选接口全 404;第三方数据商需付费 token 超出定位。
...
- 其余方向(mac 签名公证、应用内更新、其他形式内容保真如音视频卡片/合集、整本导出、多 session)均为各版**非目标**,需要时单独立项。
