...
> **R4 于 2026-09-13 增补（安哥拍板）**：新增墨问作者订阅——以作者为单位订阅、定时检查新笔记、自动下载策略与微信订阅共用；订阅页平台 Tab 切换。原非目标「不做墨问订阅」就此废止（动态/通知仍不做）。
> 当前进度见 `ROADMAP.md`，验收以本文第 6 节为准。

## 1. 一句话定义

**文库内容源从「只下微信文章」扩到「也下墨问笔记」：下载页新增「墨问笔记」tab，按用户搜索→条件拉清单→勾选批量下载，逻辑与按公众号下载微信文章同构；URL tab 同时识别墨问笔记地址；订阅页新增「墨问作者」tab，以作者为单位订阅+定时检查新笔记+可选自动下载，与微信订阅同构；复用统一文库与阅读器；启动时自动检测 mocli；文库单篇支持复制保存路径。**
...
- **R4 墨问作者订阅（2026-09-13 安哥拍板）**：
  - **以作者（墨问账号）为单位**：用户输入作者名字 → 工具返回候选账号（**带简介**，`mocli user search` 原生返回 `intro` 字段，2026-09-13 真机核实）→ 用户确认后订阅。
  - **剩余逻辑与微信公众号订阅同构**：定时检查新笔记 → 新笔记清单可见可挑 → 按策略下载入库。
  - **订阅页形态（安哥拍板）**：平台 Tab 切换——「公众号 / 墨问作者」两个 tab，各自完整的列表+搜索+检查，不混合列表。
  - **调度与设置（安哥拍板）**：与微信订阅**共用同一套**——检查频率（daily/interval）、自动下载开关一次设置管两平台；调度 tick 里分平台各查各的。
  - **发现层官方可用**：`mocli notes homepage --uid` 按作者拉清单（`--count`/`--recent`），不存在微信那个「服务端按账号封禁」问题——墨问订阅可以做完整的清单比对（微信侧因 cover 游标只能见最新一篇，墨问能翻清单）。
...
**用户目标**：像订阅微信公众号一样订阅墨问作者——定时检查新笔记、可见可挑、按策略下载入库，统一在订阅页管理。

#### R4a · 订阅页平台 Tab 与订阅管理

- 订阅页顶部加平台切换：「公众号 / 墨问作者」两个 tab（安哥拍板，不混合列表）。
- **添加订阅**：输入作者名字 → `mocli user search` 模糊搜索 → 候选卡片（**昵称 + 简介 + 主页链接**，简介直接用 mocli 返回的 `intro`，零额外请求）→ 点「订阅」确认。
- **订阅列表**：每行显示作者名 / 简介快照 / 新笔记数 / 最近检查时间；行内「检查」按钮（复用微信订阅行内检查交互）；「删除」退订。
- **重复订阅防护**：同 uid 不可重复订阅（提示「已订阅」）。
- **mocli 未装**：墨问 tab 整页显示安装指引（复用 R3 检测结果，与墨问下载 tab 同降级）。

#### R4b · 检查编排（与微信订阅同构）

```
调度/手动触发 → 对每个订阅作者：
  mocli notes homepage --uid <uid> --count 20   （按时间倒序）
      ↓
  逐条比对 publicAt ≤ watermark → 翻到水位为止就停（省请求）
      ↓
  publicAt > watermark 的并入 newNotes（按 noteId 去重，保留已下载标记）
      ↓
  自动下载开着 → 逐篇 downloadMowenNote（串行；付费 unavailable 如实；已在文库 skip）
      ↓
  检查成功才推进 watermark（到本轮见到的最大 publicAt）；失败保留失败类型不推
```

- **失败语义平移 v0.10.5 教训**：mocli 失败（未装/鉴权/网络）如实归集到该作者名下，**不得降级为「没有新笔记」**；watermark 只在检查成功后推进。
- **检查日志**：复用微信订阅检查日志通道，`platform` 字段区分来源；墨问侧逐号新笔记明细与下载交付明细与微信同形态。
- **登录态无关**：墨问检查只依赖 mocli 安装+认证（R3 检测），与微信读书登录态完全独立——微信侧未登录不影响墨问检查，反之亦然。

#### R4c · 数据与调度

- **存储**：库根下 `mowen-subscriptions.json`（与微信订阅分文件——两边 schema 语义不同：fakeid/watermark（createTime）vs uid/watermark（publicAt），硬合一个文件互相污染）。条目：

  | 字段 | 说明 |
  |------|------|
  | `uid` | 作者 uid（主键） |
  | `name` / `intro` | 订阅时快照（intro 展示用，作者改名不追溯） |
  | `subscribed` | true |
  | `watermark` | 已见最新 publicAt（unix 秒；`publicAt > watermark` 即新） |
  | `lastCheckedAt` | unix ms |
  | `newNotes` | `{ noteId, title, publicAt, url, status: 'pending'\|'downloaded'\|'ignored' }[]` 待处理新笔记（状态语义对齐微信 `ArticleRef`，下载/忽略后状态回填、不被下次检查重置） |

- **调度共用（安哥拍板）**：复用微信订阅调度器（daily/interval + 防重入 + 抖动），一个 tick 里微信（有订阅且登录态可用才查）与墨问（有订阅且 mocli 可用才查）各自独立执行、独立归集失败。
- **自动下载开关共用**：沿用微信订阅的自动下载设置项；关时新笔记只入 `newNotes` 待用户挑。
- **CLI 命令**（对齐微信订阅命令组）：

  | 命令 | 用途 |
  |------|------|
  | `wx-kit mowen subscribe --keyword 名字` | 搜作者输出候选 JSON（含 uid/name/intro），`--uid <uid> --keyword 名字` 直接订阅 |
  | `wx-kit mowen unsubscribe --uid <uid>` | 退订 |
  | `wx-kit mowen list` | 订阅列表（含 newNotes 摘要） |
  | `wx-kit mowen check-now [--uid <uid>]` | 立即检查（全量或单作者），输出逐号明细 |
...
- **R4 检查深度**：每作者每轮 `--count 20`（倒序翻到水位为止即停，通常 1 页搞定）；`--recent` 档位不用于订阅检查（水位比对已覆盖）。
- **R4 新笔记去重合并**：`newNotes` 按 noteId 合并（复用 `mergeNewRefs` 思路），已下载/已忽略的笔记不被下次检查重复置顶；单篇下载入口（行内点标题/勾选下载）复用 M58 交互模式。
- **R4 订阅时基线**：订阅成功的那一刻**不回补历史**——watermark 设为当前时间（后续新笔记才计入）；想补历史走 R2a 批量下载 tab。

## 5. 非目标

- ❌ 不集成墨问 OpenAPI 直连（spike #1 实测本机 ALB 503；且 `--show-atom` 限自己笔记，API 给不了他人全文）。
- ❌ 不做墨问**动态流 / 通知推送** UI（订阅与检查归 wx-kit——R4；动态浏览与通知仍是 mocli + skill 的事）。
...
- **M63 · R4 墨问作者订阅**（2026-09-13 增补）：mowen-subscriptions.json + 水位比对/检查编排 + 订阅页墨问 tab（搜索订阅/列表/行内检查）+ CLI subscribe/unsubscribe/list/check-now + 调度共用接线
