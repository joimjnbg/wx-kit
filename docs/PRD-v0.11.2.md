...
- [ ] `<note uuid>` 占位标签替换：含引用笔记下载后，正文原位置出现引用卡片（标题/摘要/
      作者/链接）；**阅读器 HTML 与 md 导出均可见**（turndown 转换卡片不丢标题）。
- [ ] 单条子笔记元信息拉取限速 0.5s/篇；父笔记落库不因子笔记失败而失败。
- [ ] 付费子笔记卡片如实标注「付费笔记，标题不可见」，链接保留（真机样本
      -Bh35Ogyfr7OQTs-GGsCu）。
- [ ] 尾部「引用笔记（N 篇）」追加块不再出现；warnings 保留引用提示一条。
- [ ] expandRefs 展开时子笔记元信息不重复请求（同一 uuid 只拉一次）。
- [ ] 元信息整轮失败：父笔记正常下载，卡片回退「标题获取失败」形态 + warning（单测注入
      fetchJson 失败模拟）。
- [ ] 单测：note/show fixture 含 `<note uuid>` 的正文替换、付费 400、元信息失败回退、
      expandRefs 复用请求（spy 计数）。
