...
- [ ] Segmented 切换：按用户模式行为与 v0.11.1 完全一致（e2e 既有断言不改动全过）。
- [ ] 按关键词搜索：结果表含标题/摘要/作者/发表/阅读列；作者名点击切回按用户模式并展开
      该作者清单（条件行出现）。
- [ ] 模式切换清空结果与勾选状态（e2e 断言旧结果不残留）。
- [ ] 搜索结果下载：勾选 → 下载选中 → 进度/历史/结果区与清单下载同表现；付费默认不选、
      可手动勾选（下载时如实报 unavailable，与 M61 语义一致）。
- [ ] 「含引用子笔记」勾选在搜索结果表同样可用（分批提交语义不变）。
- [ ] mocli 未装时两种模式的降级指引条均正常（MOCLI_NOT_FOUND handle 共用）。
- [ ] 单测：searchNotes 返回含 authorName（users 映射拼接）；无 users 键时 authorName 空
      不崩。
